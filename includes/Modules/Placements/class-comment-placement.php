...
class Comment_Placement implements Placement_Interface {
	...
	public function render_options( $ad_id, $data ) {
		...
		<div class="wbam-placement-extra">
			...
			<select id="wbam_comment_position" name="wbam_data[comment_position]">
		...
		<div class="wbam-placement-extra" data-wbam-show-when="wbam_comment_position:between_comments"<?php echo ( 'between_comments' !== $position ) ? ' hidden' : ''; ?>>
			<label for="wbam_comment_after"><?php esc_html_e( 'Show after every', 'wb-ads-rotator-with-split-test' ); ?></label>
			<input type="number" id="wbam_comment_after" class="small-text" name="wbam_data[comment_after]" value="<?php echo esc_attr( $after_count ); ?>" min="1" max="50" />
