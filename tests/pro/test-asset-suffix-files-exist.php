<?php
/**
 * Every PRO-plugin CSS/JS asset ships both its source and its .min sibling.
 *
 * wbam_pro_asset_url() decides which file a handle loads by switching on
...
class Test_Asset_Suffix_Files_Exist extends Pro_Test_Case {
	 ...
	 * Vendor libraries shipped minified-only (assets/vendor/chart.umd.min.js)
	...
	private static function relative_paths(): array {
		...
		foreach ( array( 'css', 'js' ) as $dir ) {
			foreach ( (array) glob( WBAM_PRO_PATH . 'assets/' . $dir . '/*.' . $dir ) as $file ) {
				$name = basename( $file );
	...
	/**
	 * Resolve wbam_pro_asset_url() to a filesystem path, forcing the suffix
	 * via the filter rather than defining SCRIPT_DEBUG globally.
	 */
	private function resolved_path( string $relative_path, bool $script_debug_on ): string {
		...
		add_filter( 'wbam_pro_asset_suffix', $force );
		$url = wbam_pro_asset_url( $relative_path );
		remove_filter( 'wbam_pro_asset_suffix', $force );

		return WBAM_PRO_PATH . substr( $url, strlen( WBAM_PRO_URL ) );
	}
